...
color_map = [list(xkcd_colors.keys())[k] for k in np.random.choice(range(len(xkcd_colors)), 28, replace=False)]
color_map = [
    "#00998F",
    "#C20088",
    "#0075DC",
...
feature_map = {
    "mimic": feature_map_mimic,
    "simulation": feature_map_simulation,
    "simulation_spike": feature_map_simulation,
}
    ...
    def train(self, n_epochs, learn_rt=False):
        ...
        if not learn_rt:
            train_model(
                ...
                n_epochs,
                self.device,
                self.experiment + "_" + self.model_type,
                data=self.data,
            )
            # Evaluate performance on held-out test set
            _, _, auc_test, correct_label, test_loss = test(self.test_loader, self.model, self.device)
        ...
        **kwargs
        ...
        if "simulation" in self.data:
            if not self.learned_risk:
                self.risk_predictor = lambda signal, t: logistic(
                    2.5 * (signal[0, t] * signal[0, t] + signal[1, t] * signal[1, t] + signal[2, t] * signal[2, t] - 1)
                )
            else:
                self.risk_predictor = EncoderRNN(
                    self.input_size,
                    hidden_size=configs["encoding_size"],
                    rnn=configs["rnn_type"],
                    regres=True,
                    data=data,
                )
        ...
        for test_sample_ind, test_sample in enumerate(samples_to_analyze):
            ...
            for tttt in range(matrix_test_dataset.shape[2]):
                exp = self.explainer.explain_instance(
                    np.mean(matrix_test_dataset[test_sample, :, : tttt + 1], axis=1),
                    self.predictor_wrapper,
                    labels=["feature " + str(i) for i in range(len(self.feature_map))],
                    top_labels=2,
                )
    ...
    def train(self, n_epochs, learn_rt=False):
        ...
        if self.baseline_method == "lime":
            self.explainer = lime.lime_tabular.LimeTabularExplainer(
                ...
                feature_names=self.feature_map + ["gender", "age", "ethnicity", "first_icu_stay"],
...
class FeatureGeneratorExplainer(Experiment):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.generator_type == "RNN_generator":
            ...
            self.generator = JointFeatureGenerator(feature_size, data=data).to(
                self.device
            )  # TODO setup the right encoding size
        ...
        if "simulation" in self.data:
            if not self.learned_risk:
                ...
                if self.data == "simulation_spike":
                    if self.predictor_model == "RNN":
                        self.risk_predictor = EncoderRNN(
                            feature_size, hidden_size=50, rnn="GRU", regres=True, return_all=False, data=data
                        )
                    ...
                    if self.predictor_model == "RNN":
                        self.risk_predictor = EncoderRNN(
                            feature_size, hidden_size=100, rnn="GRU", regres=True, return_all=False, data=data
                        )
                    elif self.predictor_model == "attention":
                        self.risk_predictor = AttentionModel(feature_size=feature_size, hidden_size=100, data=data)
                    self.risk_predictor_attention = AttentionModel(
                        feature_size=feature_size, hidden_size=100, data=data
                    )
            self.feature_map = feature_map_simulation
            self.risk_predictor = self.risk_predictor.to(self.device)
            ...
            if self.data == "mimic":
                if self.predictor_model == "RNN":
                    self.risk_predictor = EncoderRNN(
                        self.input_size, hidden_size=150, rnn="GRU", regres=True, data=data
                    )
                elif self.predictor_model == "attention":
        ...
        if self.data == "ghg":
            ...
            for sub_ind, sample_ID in enumerate(samples_to_analyze):
                ...
                for s, sub_group in enumerate(sub_features):
                    _, imp = self._get_feature_importance_FIT(
                        signals, sig_ind=sub_group, n_samples=50, learned_risk=self.learned_risk
                    )
                    imps[s, :] = imp
                ...
                with open(
                    os.path.join(self.output_path, self.predictor_model, "top_features_" + str(sample_ID) + ".pkl"),
                    "wb",
                ) as f:
                    pkl.dump(
                        {"feauture_set": top_subfeatures, "importance": top_subfeatures_scores},
                        f,
    ...
    def run(self, train, n_epochs, samples_to_analyze, plot=True, **kwargs):
        ...
        if train and self.generator_type != "carry_forward_generator":
            ...
            if "simulation" in self.data:
                if self.data == "simulation_spike":
                    ...
                    with open(os.path.join("./data/simulated_spike_data/gt_test.pkl"), "rb") as f:
                        gt_importance = pkl.load(
                            f
                        )  # Type dmesg and check the last few lines of output. If the disc or the connection to it is failing, it'll be noted there.load(f)
                        # print(gt_importance)
                else:
                    with open(os.path.join("./data/simulated_data/state_dataset_importance_test.pkl"), "rb") as f:
                        gt_importance = pkl.load(f)
            ...
            if self.data == "ghg":
                ...
                signal_scaled = self.patient_data.scaler_x.inverse_transform(
                    np.reshape(signal.cpu().detach().numpy(), [len(samples_to_analyze), -1])
                )
            ...
            if not "simulation" in self.data:  # noqa: E713
                ...
                if not self.learned_risk:
                    ...
                    for kk, i in enumerate(samples_to_analyze):
                        ...
                        grad_x0 = np.array(
                            [5 * out[tt] * (1 - out[tt]) * (gt_importance[i, tt] == 0) * sample[0, tt] for tt in tvec]
                        )
                        grad_x1 = np.array(
                            [5 * out[tt] * (1 - out[tt]) * (gt_importance[i, tt] == 1) * sample[1, tt] for tt in tvec]
                        )
                        grad_x2 = np.array(
                            [5 * out[tt] * (1 - out[tt]) * (gt_importance[i, tt] == 2) * sample[2, tt] for tt in tvec]
                        )
                    ...
                    for t_ind, t in enumerate(tvec):
                        ...
                        for s in range(len(samples_to_analyze)):
                            out[s].backward(retain_graph=True)
                            sensitivity_analysis[s, :, t_ind] = (
                                signal_t.grad.data[s, :, t_ind].cpu().detach().numpy()
                            )  # [:,0]
                        signal_t.grad.data.zero_()
                    self.risk_predictor.eval()
            ...
            if self.data == "ghg":
                ...
                for sub_ind, sample_ID in enumerate(samples_to_analyze):
                    ...
                    (
                        ...
                        sample_ID,
                        signals_to_analyze,
                        sensitivity_analysis[sub_ind, :, :],
                        data=self.data,
                        plot=plot,
                        gt_importance_subj=gt_importance[sample_ID] if "simulation" in self.data else None,
                        lime_imp=lime_imp,
                        tvec=tvec,
                        cv=cv,
                    )
        ...
        return (
            np.array(all_FIT_importance),
            np.array(all_AFO_importance),
            np.array(all_FO_importance),
            np.array(all_lime_importance),
    ...
    def plot_baseline(
        ...
        **kwargs
        ...
        for i, sig_ind in enumerate(signals_to_analyze):
                ...
                if "joint" in self.generator_type:
                    ...
                    if data == "mimic":
                        self.generator.load_state_dict(
                            torch.load(
                                os.path.join(
                                    self.ckpt_path, "%s_%s.pt" % (feature_map_mimic[sig_ind], self.generator_type)
                                )
                            )
                        )
            ...
            t0 = time.time()
            label, importance[i, :] = self._get_feature_importance_FIT(
                signals, sig_ind=[sig_ind], n_samples=10, learned_risk=self.learned_risk
            )
            t1 = time.time()
            FIT_exe_time.append(t1 - t0)

            t0 = time.time()
            _, importance_occ[i, :], _, std_predicted_risk_occ[i, :] = self._get_feature_importance(
                signals,
                sig_ind=sig_ind,
                n_samples=10,
            ...
            t0 = time.time()
            _, importance_occ_aug[i, :], _, std_predicted_risk_occ_aug[i, :] = self._get_feature_importance(
                signals,
                sig_ind=sig_ind,
                n_samples=10,
            ...
            attention_importance[i, :] = (
                self.risk_predictor_attention.get_attention_weights(signals.unsqueeze(0))
                .detach()
                .cpu()
                .numpy()[0, 1:]
                .reshape(-1,)
        ...
        with open(
            os.path.join(self.output_path, self.predictor_model, "results_" + str(subject) + "cv_" + str(cv) + ".pkl"),
            "wb",
        ) as f:
            pkl.dump(
                {
                    "FFC": {"imp": importance, "std": std_predicted_risk},
                    "Suresh_et_al": {"imp": importance_occ, "std": std_predicted_risk_occ},
                    "AFO": {"imp": importance_occ_aug, "std": std_predicted_risk_occ_aug},
                    "Sens": {"imp": sensitivity_analysis_importance, "std": []},
                    "lime": {"imp": lime_imp, "std": []},
                    "attention": {"imp": attention_importance, "std": []},
                    "gt": gt_importance_subj,
                },
                f,
                protocol=pkl.HIGHEST_PROTOCOL,
            )
        ...
        imps = np.stack(
            [
                ...
                importance_occ_aug,
                importance_occ,
                np.abs(sensitivity_analysis_importance),
        ...
        std_imps = np.stack(
            [
                std_predicted_risk,
                std_predicted_risk_occ_aug,
                std_predicted_risk_occ,
                np.zeros(std_predicted_risk.shape),
                np.zeros(std_predicted_risk.shape),
            ],
            axis=0,
        )
        ...
        plot_importance(
            ...
            data,
            gt_importance_subj,
            os.path.join(self.output_path, self.predictor_model),
            self.patient_data,
        )
        return (
            max_imp_FCC,
    ...
    def _get_feature_importance_FIT(
        self, signal, sig_ind, n_samples=10, learned_risk=True, tvec=None, at_time=None, conditional=False
    ):
        self.generator.eval()
        risks = []
        importance_FIT = []
        ...
        if at_time is None:
            for t in tvec:
                if "simulation" in self.data:
                    ...
                    risk = self.risk_predictor(
                        signal[:, 0 : t + self.generator.prediction_size].view(
                            1, signal.shape[0], t + self.generator.prediction_size
                        )
                    ).item()
                ...
                for _ in range(n_samples):
                    ...
                    if conditional:
                        x_hat_t = self.generator.forward_conditional(
                            signal[:, :t].unsqueeze(0), signal[:, t], [i for i in range(len(signal)) if i != sig_ind]
                        )
                    ...
                    if "simulation" in self.data and not learned_risk:
                        ...
                        predicted_risk_FIT = self.risk_predictor(
                            predicted_signal_FIT.unsqueeze(0).to(self.device)
                        ).item()
                    generator_predicted_risks.append(predicted_risk_FIT)

                # KL divergence
                probability_subsection_FIT = torch.Tensor(
                    [1 - np.mean(generator_predicted_risks), np.mean(generator_predicted_risks)]
                )
                probability_all = torch.Tensor([(1 - risk), risk])

                div_FIT = torch.nn.KLDivLoss()(
                    torch.log(torch.Tensor(probability_all)).squeeze(0),
                    torch.Tensor(probability_subsection_FIT).squeeze(0),
                )
            ...
            if "simulation" in self.data:
                ...
                risk = self.risk_predictor(
                    signal[:, 0 : at_time + self.generator.prediction_size].view(
                        1, signal.shape[0], at_time + self.generator.prediction_size
                    )
                ).item()
            ...
            for _ in range(n_samples):
                ...
                x_hat_t_cond, _ = self.generator.forward_conditional(
                    signal[:, :at_time].unsqueeze(0), signal[:, at_time], sig_ind
                )
                ...
                if "simulation" in self.data and not learned_risk:
                    ...
                    conditional_predicted_risk = self.risk_predictor(
                        predicted_signal_conditional.unsqueeze(0).to(self.device)
                    ).item()
                conditional_predicted_risks.append(conditional_predicted_risk)

            # KL divergence
            probability_subsection = torch.Tensor(
                [1 - np.mean(conditional_predicted_risk), np.mean(conditional_predicted_risk)]
            )
            probability_all = torch.Tensor([(1 - risk), risk])
    ...
    def _get_feature_importance(
        self, signal, sig_ind, n_samples=10, mode="feature_occlusion", learned_risk=True, tvec=None
    ):
        self.generator.eval()
        ...
        for t in tvec:
            ...
            for _ in range(n_samples):
                ...
                if mode == "feature_occlusion":
                    ...
                    predicted_signal[:, t : t + self.generator.prediction_size] = torch.cat(
                        (
                            signal[:sig_ind, t : t + self.generator.prediction_size],
                            prediction.view(1, -1),
                            signal[sig_ind + 1 :, t : t + self.generator.prediction_size],
                        ),
                        0,
                    )
                elif mode == "augmented_feature_occlusion":
                    if self.risk_predictor(signal[:, 0:t].view(1, signal.shape[0], t)).item() > 0.5:
                        prediction = torch.Tensor(np.array(np.random.choice(feature_dist_0)).reshape(-1,)).to(
                            self.device
                        )
                    else:
                        prediction = torch.Tensor(np.array(np.random.choice(feature_dist_1)).reshape(-1,)).to(
                            self.device
                        )
                    predicted_signal = signal[:, 0 : t + self.generator.prediction_size].clone()
                    predicted_signal[:, t : t + self.generator.prediction_size] = torch.cat(
                ...
                if "simulation" in self.data:
                    if not learned_risk:
                        ...
                        predicted_risk = self.risk_predictor(
                            predicted_signal[:, 0 : t + self.generator.prediction_size]
                            .view(1, predicted_signal.shape[0], t + self.generator.prediction_size)
                            .to(self.device)
                        ).item()
                else:
                    predicted_risk = self.risk_predictor(
