...
from fit.TSX.utils import load_simulated_data, train_model_rt, compute_median_rank, train_model_rt_binary, \
...
from fit.TSX.explainers import RETAINexplainer, FITExplainer, IGExplainer, FFCExplainer, \
    ...
    if args.explainer == 'retain':
        ...
        if args.train:
            ...
            if args.data=='mimic' or args.data=='simulation' or args.data=='simulation_l2x':
                ...
                explainer.fit_model(train_loader, valid_loader, test_loader, lr=1e-4, plot=False, epochs=100,cv=args.cv)
            print('Total time required to train retain: ', time.time() - t0)
        else:
            model.load_state_dict(torch.load(os.path.join('./experiments/results/%s/%s_%d.pt' % (data_name, 'retain', args.cv))))
    else:
        if not args.binary:
            ...
            if not args.binary:
                ...
                if args.data=='mimic':
                    ...
                    train_model_rt(model=model, train_loader=train_loader, valid_loader=valid_loader, optimizer=optimizer, n_epochs=50,
                               device=device, experiment='model', data='state',cv=args.cv)
                elif args.data=='mimic_int':
        ...
        model.load_state_dict(torch.load(os.path.join('./experiments/results/%s/%s_%d.pt' % (data_name, 'model',args.cv))))
        ...
        if args.explainer == 'fit':
            if args.generator_type=='history':
                ...
                if args.train:
                    ...
                    generator.load_state_dict(torch.load(os.path.join('./experiments/results/%s/%s_%d.pt' % (data_name, 'joint_generator',args.cv))))
                    if args.data=='mimic_int' or args.data=='simulation_spike':
            ...
            if args.train:
                ...
                generator.load_state_dict(torch.load(os.path.join('./experiments/results/%s/%s.pt' % (args.data, 'joint_generator'))))
                if args.data=='mimic_int' or args.data=='simulation_spike':
