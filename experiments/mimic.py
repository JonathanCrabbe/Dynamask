...
def run_experiment(cv: int = 0, area: float = 0.1):
    ...
    # Load the model:
    model = StateClassifier(feature_size=N_features, n_state=2, hidden_size=200,
                            rnn='GRU', return_all=True, device=device)
    ...
    for k, x_test in enumerate(torch.unbind(X_test)):
        ...
        mask.fit(X=x_test, f=f, loss_function=log_loss_target, keep_ratio=area, target=Y[k],
                 learning_rate=1.0, size_reg_factor_init=0.1, size_reg_factor_dilation=10000,
                 initial_mask_coeff=0.5, n_epoch=1000, momentum=1.0, time_reg_factor=0)
