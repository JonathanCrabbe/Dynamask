...
def run_experiment(cv: int = 0, N_ex: int = 10, T: int = 50, N_features: int = 50, N_select: int = 5,
    ...
    :param N_select: Number of time steps that are truly salient
    ...
    for k in range(N_ex): # We compute the attribution for each individual time series
        ...
        def f(input):
            ...
            output[t_rand:t_rand+N_select, int(N_features / 4):int(3 * N_features / 4)] = \
                input[t_rand:t_rand+N_select, int(N_features / 4):int(3 * N_features / 4)]
        ...
        mask_group.fit(f=f, X=x, area_list=np.arange(0.001, 0.051, 0.001), loss_function=mse,
                       n_epoch=1000, size_reg_factor_dilation=1000, size_reg_factor_init=1,
                       learning_rate=1)
